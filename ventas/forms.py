...
class VentaForm(forms.ModelForm):
    codigo_cliente = forms.CharField(
...
class DevolucionForm(forms.ModelForm):
    ...
    tipo_devolucion = forms.ChoiceField(
        choices=Devolucion.TIPO_DEVOLUCION_CHOICES,
        widget=forms.Select(attrs={'class': 'form-select'}),
    ...
    presenta_comprobante = forms.BooleanField(
        widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}),
        label='Presenta comprobante',
        required=False,
    )
    ...
    class Meta:
        model  = Devolucion
        fields = [
            'motivo', 'tipo_devolucion', 'observaciones',
            'restaurar_stock', 'presenta_comprobante',
        ]