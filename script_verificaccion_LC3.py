        ...
        for f in files:
            ...
            if f.endswith('.css'):
                ...
                try:
                    ...
                    ignored_ranges = []
                    # Ignorar bloques de script
                    for match in re.finditer(r'<script\b[^>]*>([\s\S]*?)</script>', content, re.IGNORECASE):
                        start_pos = match.start()
                        end_pos = match.end()
                        start_line = content[:start_pos].count('\n') + 1
                        end_line = content[:end_pos].count('\n') + 1
                        ignored_ranges.append((start_line, end_line))
                    # Ignorar bloques de style
                    for match in re.finditer(r'<style\b[^>]*>([\s\S]*?)</style>', content, re.IGNORECASE):
                        start_pos = match.start()
                        end_pos = match.end()
                        start_line = content[:start_pos].count('\n') + 1
                        end_line = content[:end_pos].count('\n') + 1
                    ...
                    if not is_email_or_pdf:
                        for idx, line in enumerate(lines, start=1):
                            # Ignorar si la línea está dentro de un bloque script o style
                            is_ignored = False
                            for s_line, e_line in ignored_ranges:
                            ...
                            if matches:
                                for match in matches:
                                    ...
                                    if "{{" in match or "{%" in match:
                                        continue
                                    # Ignorar style="display:none" o display:none; (falsa alarma común para elementos ocultos)
                                    style_val = match.split('=', 1)[1].strip(' "\';').replace(" ", "").lower()
                                    if style_val == "display:none":
                                        continue
                                    inline_styles += 1
...
def check_documentation():
    ...
    for root, dirs, files in os.walk(BASE_DIR):
        ...
        for f in files:
            path = os.path.join(root, f)
            if f.endswith('.py'):
                if f in ['__init__.py', 'manage.py', 'wsgi.py', 'asgi.py', 'apps.py', 'settings.py']:
                    continue
                try:
                    with open(path, 'r', encoding='utf-8') as file:
                        content = file.read()
                ...
                try:
                    with open(path, 'r', encoding='utf-8') as file:
                        content = file.read()
                        if 'function' in content or '=>' in content or 'class ' in content:
                            js_files += 1
