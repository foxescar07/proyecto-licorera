...
def lista_usuarios(request):
    ...
    usuarios = Perfil.objects.select_related('user').order_by(
        '-user__date_joined'
    )
...
def editar_usuario(request, pk):
    ...
    if request.method == 'POST':
        ...
        perfil.user.save(update_fields=[
            'first_name',
            'last_name',
            'email'
        ])
        ...
        messages.success(
            request,
            f'Usuario {perfil.usuario} actualizado correctamente.'
        )
...
def solicitar_recuperacion(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            _enviar_correo(
                perfil.user.email,
                'Recuperación de contraseña - CYS Ltda',
                cuerpo
            )
            ...
            if es_ajax:
                return JsonResponse({
                    'ok': True,
                    'mensaje': f'Enlace enviado a {perfil.user.email}'
                })

            messages.success(
                request,
                f'Enlace enviado a {perfil.user.email}'
            )
